...
# 定义题目和答案
questions_answers = {
    "1. 在光滑斜面上，放有物体X和物体Y，一根轻质绳将两个物体连接在一起。如图所示，将沿着斜面向上的恒力F作用在物体Y上。如果斜面倾角变大，绳的张力会有怎样的变化？A.不变 B.变小 C.变大": 
    """正确答案是A不变。\n\n在θ不变的情况下对两个物块整体沿着斜面方向进行受力分析，再根据牛顿第二定律算出两个物块沿着斜面的共同加速度 F - (mX + mY)gsinθ = (mX + mY)a；
然后再用隔离法研究物块X的受力情况，根据牛顿第二定律列出方程 F - mXgsinθ = mX a；带入共同加速度 a 可以算出绳的张力 T = mX * F/ (mX + mY) 。""",
...
# 输入学生 ID
student_id = st.text_input("请输入你的姓名:", "")
